...
State: GAP TEST SOURCE ACCEPTED — LUNA EXPECTED RED AUTHORIZED
...
Active handoff: [CODEX_LUNA_BBGO_PAY_001_GAP_EXPECTED_RED_01.md](CODEX_LUNA_BBGO_PAY_001_GAP_EXPECTED_RED_01.md)
...
inventory. Luna may run only the active handoff's focused expected-red command, capture
its evidence, and integrate exactly the three tests while leaving all production dirty.
No production correction, broad execution, module/lock, public network, wallet, rate,
transaction, hardware, device, release binary, or SBOM work is authorized.
