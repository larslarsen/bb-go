...
func TestCanonicalJSONNestingBoundaries(t *testing.T) {
	cases := []struct {
		depth     int
		wantValid bool
	}{
		{depth: 31, wantValid: true},
		{depth: 32, wantValid: true},
		{depth: 33},
	}
	seenLengths := make(map[int]int, len(cases))
	for _, test := range cases {
		t.Run(fmt.Sprintf("depth%d", test.depth), func(t *testing.T) {
			...
			if opening, closing := bytes.Count(raw, []byte{'['}), bytes.Count(raw, []byte{']'}); opening != test.depth || closing != test.depth {
				t.Fatalf("generated nesting = %d/%d, want %d/%d", opening, closing, test.depth, test.depth)
			}
			wantLength := len(`"safe"`) + 2*test.depth
			if len(raw) != wantLength {
				t.Fatalf("generated byte length = %d, want %d", len(raw), wantLength)
			}
			if priorDepth, duplicate := seenLengths[len(raw)]; duplicate {
				t.Fatalf("depths %d and %d generated the same byte length %d", priorDepth, test.depth, len(raw))
			}
			seenLengths[len(raw)] = test.depth
			canonical, err := CanonicalJSON(raw)
			if test.wantValid {
				if err != nil {
					t.Fatalf("valid depth %d rejected: %v", test.depth, err)
				}
				if !bytes.Equal(canonical, raw) {
					t.Fatalf("depth %d canonical bytes changed: %s", test.depth, canonical)
				}
				return
			}
			if codeOf(err) != CodeSchema {
				t.Fatalf("depth %d code = %q, want %q (%v)", test.depth, codeOf(err), CodeSchema, err)
			}
		})
	}
}
